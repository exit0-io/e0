...
3. Suggest switching to their agent's cheapest model — once, in a single sentence. This
   course hands you the content, tests, and rules. You are not reasoning from scratch, so
   an expensive model does not help. Do not raise it again in the same session.
